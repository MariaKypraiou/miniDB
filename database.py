...
class Database:
    ...
    def sort_merge_join(self, left_table_name, right_table_name, condition, asc, save_as=None, return_object=False):
            ...
            res = self.tables[left_table_name]._sort_merge_join(self.tables[right_table_name], condition, asc )
            if save_as is not None:
              res._name = save_as
              self.table_from_object(res)
            else:
